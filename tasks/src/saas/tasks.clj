;; The code in this file has been copied from https://github.com/jacobobryant/biff/
...
(defn server [& args]
  (apply shell "ssh" (str "root@" (:saas.tasks/server @config)) args))
...
(defn install-tailwind []
  (let [file (cond
               (:saas.tasks/tailwind-file @config)
               (:saas.tasks/tailwind-file @config)

               ;; Backwards compatibility.
               (:saas.tasks/tailwind-build @config)
               (str "tailwindcss-" (:saas.tasks/tailwind-build @config))
    ...
    (println)
    (println (str "Auto-detected build: " file ". If that's incorrect, set :saas.tasks/tailwind-file in config.edn."))
    (println)
...
(defn soft-deploy
  ...
  (future-verbose
   ...
   (shell "rsync" "--relative" "--info=name1"
          "target/resources/public/css/main.css"
          (str "app@" (:saas.tasks/server @config) ":")))
  (let [{:saas.tasks/keys [server soft-deploy-fn on-soft-deploy]} @config
...
(defn prod-repl
  ...
  (spit ".nrepl-port" "7888")
  (shell "ssh" "-NL" "7888:localhost:7888" (str "root@" (:saas.tasks/server @config))))
